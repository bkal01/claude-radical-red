...
The first `apply_team()` must construct its one Pokemon. This task uses one party slot.
...
The initial call must contain exactly one member in slot 0. After setup, use the active team returned by `team()` to determine
