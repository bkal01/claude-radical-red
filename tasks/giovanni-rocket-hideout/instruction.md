...
task's inclusive level cap of 47 applies. Keep notes,
...
Its `minimum_level` must not exceed this task's inclusive level cap of 47. Initial
team members must each specify an
integer `level` from 1 through 47. The level can be lower than the cap. Initial
...
`learnsets.json[species_id]`. This task has an inclusive level cap of 47:
level-up moves are valid only when their required level is 47 or lower. Moves
