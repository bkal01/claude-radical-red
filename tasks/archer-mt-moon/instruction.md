...
task's inclusive level cap of 22 applies. Keep notes,
...
Its `minimum_level` must not exceed this task's inclusive level cap of 22. Initial
team members must each specify an
integer `level` from 1 through 22. The level can be lower than the cap. Initial
...
`learnsets.json[species_id]`. This task has an inclusive level cap of 22:
level-up moves are valid only when their required level is 22 or lower. Moves
