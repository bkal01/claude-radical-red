...
Defeat the configured opposing Pokemon team in a Tag Doubles battle. You must construct your player team before
...
task's inclusive level cap of 56 applies. Keep notes,
...
The first `apply_team()` must construct its three Pokemon. This task uses three party slots.
...
their level-cap-eligible evolutions. Mega forms are excluded.
Its `minimum_level` must not exceed this task's inclusive level cap of 56. Initial
team members must each specify an
integer `level` from 1 through 56. The level can be lower than the cap. Initial
...
`learnsets.json[species_id]`. This task has an inclusive level cap of 56:
level-up moves are valid only when their required level is 56 or lower. Moves
...
The initial call must contain exactly three members in slots 0 through 2. After setup, use the active team returned by `team()` to determine
