...
supplied player team's abilities and have the configured episode budget to win.
Use only the battle-server MCP tools for game interaction; do not try to access
the ROM, emulator, or battle internals.

Before acting, call `team()` to inspect the roster and its current abilities.
...
- `apply_team(team)` updates the team's abilities and starts the next episode.
...
This task permits Ability updates. Call `apply_team()` during a live
...
that slot. Every member must include `ability_id` exactly as
shown above.
