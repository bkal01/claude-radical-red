...
task's inclusive level cap of 44 applies. Keep notes,
...
The first `apply_team()` must construct all five Pokemon. This task uses five party slots.
...
Its `minimum_level` must not exceed this task's inclusive level cap of 44. Initial
team members must each specify an
integer `level` from 1 through 44. The level can be lower than the cap. Initial
...
`learnsets.json[species_id]`. This task has an inclusive level cap of 44:
level-up moves are valid only when their required level is 44 or lower. Moves
...
The initial call must contain exactly five members, one for each slot from 0
through 4. After setup, use the active team returned by `team()` to determine
